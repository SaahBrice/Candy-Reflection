...
func load_scenes_in_level():
	var new_scene = ScenesContainer.scenes_level1[SceneController.level1_completed_levels].instance()
...
func levelCompleted(level):
	...
	if level == 24:
		SceneController.present_level = 1
		SceneController.level1_completed = true
		SceneController.new_scene = home_screen
		yield(get_tree().create_timer(1),"timeout")
		Sounds.sound_collection.rain_thunder.stop()
		get_tree().change_scene("res://scenes/Message_scene.tscn")
	SceneController.level1_completed_levels = level
