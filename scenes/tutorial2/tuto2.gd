...
func _process(delta: float) -> void:
	...
	#mirror_position_h=mirror1[0]
	#mirror_position_w=mirror1[1]
	#mirror_axis=mirror1[2]
	#mirror_size=mirror1[3]
var hint_tiles = []
		...
		for x in range(h2):
			for y in range(w2):
				if pB_1[x][y] == 1 and pB_2[h2-x-1][y] == 0:
					...
					terminated = false
	if terminated:
		Matrix.number_of_moves = hint_tiles.size()
		print(hint_tiles.size())
		print('terminated')
